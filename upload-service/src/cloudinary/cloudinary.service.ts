...
export class CloudinaryService {
  ...
  async uploadFile(file: Express.Multer.File, retries = 0): Promise<CloudinaryResponse> {
    try {
      ...
      if (this.isTransientError(error) && retries < this.MAX_RETRIES) {
        this.logger.warn(`Retrying upload for ${file.originalname}, attempt ${retries + 1}`);
        await new Promise(resolve => setTimeout(resolve, this.RETRY_DELAY));
        return this.uploadFile(file, retries + 1);
      }

      const customError = new Error(`Failed to upload ${file.originalname}: ${error.message}`);
  ...
  private isTransientError(error: any): boolean {
    ...
    return transientCodes.includes(error.code) || error.message.includes('timeout');
  }
}
