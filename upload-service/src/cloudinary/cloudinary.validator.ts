...
export class CloudinaryValidator {
  ...
  private static readonly ALLOWED_VIDEO_TYPES = ['video/mp4', 'video/mpeg', 'video/webm'];
  ...
  static validateFile(file: Express.Multer.File): void {
    ...
    if (!isImage && !isVideo) {
      throw new BadRequestException('Unsupported file type. Only images (JPEG, PNG, GIF) and videos (MP4, MPEG, WEBM) are allowed');
    }
