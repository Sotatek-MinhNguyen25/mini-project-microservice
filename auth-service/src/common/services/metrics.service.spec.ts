...
describe('MetricsService', () => {
  ...
  describe('calculateCpuUsage', () => {
    ...
    it('should use correct constants for calculation', () => {
      const result = service.calculateCpuUsage();

      // The result should be calculated using the constants
      expect(result).toBeGreaterThanOrEqual(0);
      expect(result).toBeLessThanOrEqual(APP_CONSTANTS.CPU.MAX_PERCENTAGE);
    });
  ...
  describe('calculateErrorRate', () => {
    ...
    it('should use correct constants for error rate calculation', () => {
      service.incrementRequestCount();
      service.incrementRequestCount();

      const result = service.calculateErrorRate();
      const expectedErrorCount =
        Math.floor(2 * APP_CONSTANTS.METRICS.GET_404_RATIO) +
        Math.floor(2 * APP_CONSTANTS.METRICS.POST_500_RATIO);
  ...
  describe('generatePrometheusMetrics', () => {
    ...
    it('should include correct help text', () => {
      ...
      expect(result).toContain(
        `HELP ${METRICS_CONSTANTS.METRIC_NAMES.UP} ${METRICS_CONSTANTS.METRIC_HELP.UP}`,
      );
      expect(result).toContain(
