...
export class KafkaConsumerService {
  ...
  async handleWelcomeEmail(@Payload() payload: { email: string; username: string }) {
    try {
      ...
      this.logger.error(`Error processing ${KAFKA_MESSAGE_TYPES.WELCOME_EMAIL} message: ${error}`);
      throw error;
    }
  }
  ...
  async handlePasswordResetEmail(@Payload() payload: { email: string; resetToken: string }) {
    try {
      ...
      this.logger.error(`Error processing ${KAFKA_MESSAGE_TYPES.PASSWORD_RESET_EMAIL} message: ${error}`);
      throw error;
    }
  }
  ...
  async handleVerifyRegisterEmail(@Payload() payload: { email: string; otp: string }) {
    try {
      this.logger.log(`Processing handleVerifyRegisterEmail message for ${payload.email}`);
      ...
      this.logger.error(`Error processing handleVerifyRegisterEmail message: ${error}`);
      throw error;
    }
  }
}
