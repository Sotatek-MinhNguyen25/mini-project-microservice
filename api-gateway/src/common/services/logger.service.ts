...
export class AppLoggerService {
  ...
  logRequest(method: string, endpoint: string, userId?: string, additionalData?: Record<string, any>): void {
  ...
  logResponse(method: string, endpoint: string, statusCode: number, responseTime?: number): void {
    this.info(`${method} ${endpoint} - ${statusCode}${responseTime ? ` (${responseTime}ms)` : ''}`, {
      method,
      endpoint,
      statusCode,
      responseTime,
